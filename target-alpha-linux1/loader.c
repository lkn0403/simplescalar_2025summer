...
#include <bfd.h>
#else /* !BFD_LOADER */
#include "target-alpha-linux/ecoff.h"
#include "target-alpha-linux/elf64.h"
#endif /* BFD_LOADER */
...
//  ld_text_base = MD_SWAPQ(phdr.text_start);
  ...
  ld_text_size = ld_text_base = 0;
  ld_data_size = ld_data_base = 0;
  ...
  for (i = 0; i < fhdr.e_shnum; i++)
  {
    ...
    {
      ld_text_size = MD_SWAPQ(shdr.sh_size);
      ld_text_base = MD_SWAPQ(shdr.sh_addr);
//      ld_prog_entry = shdr.sh_addr;
    }
    // store size and base of data segment
    if (!strcmp(".data", shdr_strs + shdr.sh_name))
    {
      ld_data_size = MD_SWAPQ(shdr.sh_size);
      ld_data_base = MD_SWAPQ(shdr.sh_addr);
    }
...
//    fseek(fobj, sizeof(struct ecoff_filehdr) + MD_SWAPH(fhdr.f_opthdr), 0);
	...
	printf ("shdr.s_flags: %x\n", MD_SWAPW(shdr.s_flags));
  ...
  uint64_t stack_base = (ld_text_base - 1024*1024*8) & 0xFFFFFFFFFFFF0000LL;
  uint64_t stack_min = ld_stack_base;
  ...
  for (i = 0; argv[i]; i++)
  {
    // write argv pointer
    mem_bcopy(mem_access, mem, Write,
		  /*buf*/argv_array_base + (i*8), &addr, 8);
//    memory->bcopy(true, argv_array_base + (i*8), &addr, 8);
    // write argv data
    mem_bcopy(mem_access, mem, Write,
		  /*buf*/addr, argv[i], strlen(argv[i])+1);
//    memory->bcopy(true, addr, argv[i], strlen(argv[i])+1);
  ...
  // null terminate argv array 
   mem_bcopy(mem_access, mem, Write,
		  /*buf*/argv_array_base + (i*8), &zero, 8);
//   memory->bcopy(true, argv_array_base + (i*8), &zero, 8); 
  ...
  for (i = 0; envp[i]; i++)
  {
    // write envp pointer
    mem_bcopy(mem_access, mem, Write,
		  /*buf*/envp_array_base + (i*8), &addr, 8);
//    memory->bcopy(true, envp_array_base + (i*8), &addr, 8);
    // write envp data
    mem_bcopy(mem_access, mem, Write,
		  /*buf*/addr, envp[i], strlen(envp[i])+1);
//    memory->bcopy(true, addr, envp[i], strlen(envp[i])+1);
  ...
  // null terminate argv array 
  mem_bcopy(mem_access, mem, Write,
		  /*buf*/envp_array_base + (i*8), &zero, 8);
...
//  uint64_t pc = ehdr.e_entry;
  ...
  /* set up a local stack pointer, this is where the argv and envp
     data is written into program memory */
  ld_stack_base = (ld_text_base - 1024*1024*8) & 0xFFFFFFFFFFFF0000LL;
