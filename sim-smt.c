...
static unsigned int fetch_icount[MAX_THREAD];
static unsigned int ruu_icount[MAX_THREAD];
  ...
  if (cache_dl2)
    {
      ...
      lat = cache_access(cache_dl2, cmd, tid, baddr, NULL, bsize,
...
if (cache_il2)
    {
      ...
      lat = cache_access(cache_il2, cmd, tid, baddr, NULL, bsize,
			 /* now */now, /* pudata */NULL, /* repl addr */NULL);
      if (cmd == Read)
	return lat;
      else
...
static BITMAP_TYPE(MD_TOTAL_REGS, use_spec_cv[MAX_THREAD]);
static struct CV_link create_vector[MAX_THREAD][MD_TOTAL_REGS];
static struct CV_link spec_create_vector[MAX_THREAD][MD_TOTAL_REGS];

/* these arrays shadow the create vector an indicate when a register was
   last created */
static tick_t create_vector_rt[MAX_THREAD][MD_TOTAL_REGS];
static tick_t spec_create_vector_rt[MAX_THREAD][MD_TOTAL_REGS];

/* read a create vector entry */
#define CREATE_VECTOR(TID,N)        (BITMAP_SET_P(use_spec_cv[TID], CV_BMAP_SZ, (N))\
				 ? spec_create_vector[TID][N]                \
				 : create_vector[TID][N])

/* read a create vector timestamp entry */
#define CREATE_VECTOR_RT(TID,N)     (BITMAP_SET_P(use_spec_cv[TID], CV_BMAP_SZ, (N))\
				 ? spec_create_vector_rt[TID][N]             \
				 : create_vector_rt[TID][N])

/* set a create vector entry */
#define SET_CREATE_VECTOR(TID, N, L) (spec_mode[TID]                              \
				 ? (BITMAP_SET(use_spec_cv[TID], CV_BMAP_SZ, (N)),\
				    spec_create_vector[TID][N] = (L))        \
				 : (create_vector[TID][N] = (L)))
...
cv_init(void)
{
  ...
  for (i=0; i < MD_TOTAL_REGS; i++)
    {
        create_vector[tid][i] = CVLINK_NULL;
        create_vector_rt[tid][i] = 0;
        spec_create_vector[tid][i] = CVLINK_NULL;
        spec_create_vector_rt[tid][i] = 0;
    }
...
ruu_recover(int branch_index)			/* index of mis-pred branch */
{
  ...
  /* recover from the tail of the RUU towards the head until the branch index
     ...
     the RUU */
  int recover_tid = RUU[branch_index].tid;
  ...
  while (RUU_index != branch_index) {
    ...
    if (RUU[RUU_index].tid != recover_tid) {
  ...
  BITMAP_CLEAR_MAP(use_spec_cv[recover_tid], CV_BMAP_SZ);
...
ruu_writeback(void)
{
  ...
  while ((rs = eventq_next_event())) {
    ...
    if (rs->recover_inst) {
      ...
      ruu_recover(rs - RUU);
      tracer_recover(tid);
      bpred_recover(pred[tid], rs->PC, rs->stack_recover_idx);
...
lsq_refresh(void)
{
  int i, j, index, n_std_unknowns[MAX_THREAD];
  md_addr_t std_unknowns[MAX_THREAD][MAX_STD_UNKNOWNS];
  int sta_unknown[MAX_THREAD];
  for (int i=0; i<thread_num; i++) {sta_unknown[i] = 0; n_std_unknowns[i] = 0; }
...
ruu_issue(void)
{
  ...
  /* visit all ready instructions (i.e., insts whose register input
     dependenciefu0s have been satisfied, stop issue when no more instructions
			  ...
			  if (i != LSQ_head)
			      ...
			      for (;;)
				{
				  ...
				  if ((MD_OP_FLAGS(LSQ[i].op) & F_STORE)
				      && (LSQ[i].addr == rs->addr) && LSQ[i].tid == rs->tid)
				    {
				      /* hit in the LSQ */
				      load_lat = 1;
				      break;
			  ...
			  if (dtlb && MD_VALID_ADDR(rs->tid, rs->addr))
			    {
				 ...
				 initiate speculative TLB misses */
			      tlb_lat =
				cache_access(dtlb, Read, rs->tid, (rs->addr & ~3),
					     NULL, 4, sim_cycle, NULL, NULL);
			      if (tlb_lat > 1)
				events |= PEV_TLBMISS;
...
static BITMAP_TYPE(MD_NUM_IREGS, use_spec_R[MAX_THREAD]);
static md_gpr_t spec_regs_R[MAX_THREAD];

/* floating point register file */
#define F_BMAP_SZ       (BITMAP_SIZE(MD_NUM_FREGS))
static BITMAP_TYPE(MD_NUM_FREGS, use_spec_F[MAX_THREAD]);
static md_fpr_t spec_regs_F[MAX_THREAD];
...
static void
rspec_dump(int tid, FILE *stream)			/* output stream */
{
  int i;
  ...
  for (i=0; i < MD_NUM_IREGS; i++)
    {
      if (BITMAP_SET_P(use_spec_R[tid], R_BMAP_SZ, i))
	{
	  md_print_ireg(spec_regs_R[tid], i, stream);
	  fprintf(stream, "\n");
	}
    }
  ...
  for (i=0; i < MD_NUM_FREGS; i++)
    {
      if (BITMAP_SET_P(use_spec_F[tid], F_BMAP_SZ, i))
	{
	  md_print_fpreg(spec_regs_F[tid], i, stream);
	  fprintf(stream, "\n");
	}
    }
  ...
  for (i=0; i < MD_NUM_CREGS; i++)
    {
      if (BITMAP_SET_P(use_spec_C[tid], C_BMAP_SZ, i))
	{
	  md_print_creg(spec_regs_C[tid], i, stream);
	  fprintf(stream, "\n");
	}
    }
}
...
static struct spec_mem_ent *store_htable[MAX_THREAD][STORE_HASH_SIZE];

/* speculative memory hash table bucket free list */
static struct spec_mem_ent *bucket_free_list[MAX_THREAD];


/* program counter */
static md_addr_t pred_PC[MAX_THREAD];
static md_addr_t recover_PC[MAX_THREAD];

/* fetch unit next fetch address */
static md_addr_t fetch_regs_PC[MAX_THREAD];
static md_addr_t fetch_pred_PC[MAX_THREAD];
...
tracer_recover(int tid)
{
  ...
  BITMAP_CLEAR_MAP(use_spec_R[tid], R_BMAP_SZ);
  BITMAP_CLEAR_MAP(use_spec_F[tid], F_BMAP_SZ);
  BITMAP_CLEAR_MAP(use_spec_C[tid], C_BMAP_SZ);
  ...
  for (i=0; i<STORE_HASH_SIZE; i++) {
    ...
    for (ent=store_htable[tid][i]; ent; ent=ent_next) {
      ent_next = ent->next;
      ent->next = bucket_free_list[tid];
      bucket_free_list[tid] = ent;
    }
    store_htable[tid][i] = NULL;
  }
		...
		int nbytes)			/* number of bytes to access */
{
  ...
  for (prev=NULL,ent=store_htable[tid][index]; ent; prev=ent,ent=ent->next)
    {
      if (ent->addr == addr)
	{
	  ...
	  if (prev != NULL)
	    {
	      ...
	      prev->next = ent->next;
              ent->next = store_htable[tid][index];
              store_htable[tid][index] = ent;
	    }
	  break;
	}
    }
  ...
  if (!ent && cmd == Write)
    {
      ...
      if (!bucket_free_list[tid])
	{
	  /* otherwise, call calloc() to get the needed storage */
	  bucket_free_list[tid] = calloc(1, sizeof(struct spec_mem_ent));
	  if (!bucket_free_list[tid])
	    fatal("out of virtual memory");
	}
      ent = bucket_free_list[tid];
      bucket_free_list[tid] = bucket_free_list[tid]->next;
...
static void
mspec_dump(int tid, FILE *stream)			/* output stream */
{
  int i;
  ...
  for (i=0; i<STORE_HASH_SIZE; i++)
    {
      ...
      for (ent=store_htable[tid][i]; ent; ent=ent->next)
	{
	      ...
	      int idep_name)			/* input register name */
{
  ...
  head = CREATE_VECTOR(tid, idep_name);
		 ...
		 int odep_name)			/* output register name */
{
  ...
  SET_CREATE_VECTOR(tid, odep_name, cv);
}
...
#define GPR(TID, N)                  (BITMAP_SET_P(use_spec_R[TID], R_BMAP_SZ, (N))\
				 ? spec_regs_R[TID][N]                       \
				 : regs[TID].regs_R[N])
#define SET_GPR(TID,N,EXPR)         (spec_mode[TID]				\
				 ? ((spec_regs_R[TID][N] = (EXPR)),		\
				    BITMAP_SET(use_spec_R[TID], R_BMAP_SZ, (N)),\
				    spec_regs_R[TID][N])			\
...
/* floating point register accessors, NOTE: speculative copy on write storage
   provided for fast recovery during wrong path execute (see tracer_recover()
   for details on this process */
#define FPR_Q(TID, N)		(BITMAP_SET_P(use_spec_F[TID], F_BMAP_SZ, (N))\
				 ? spec_regs_F[TID].q[(N)]                   \
				 : regs[TID].regs_F.q[(N)])
#define SET_FPR_Q(TID, N,EXPR)	(spec_mode[TID]				\
				 ? ((spec_regs_F[TID].q[(N)] = (EXPR)),	\
				    BITMAP_SET(use_spec_F[TID],F_BMAP_SZ, (N)),\
				    spec_regs_F[TID].q[(N)])			\
				 : (regs[TID].regs_F.q[(N)] = (EXPR)))
#define FPR(TID, N)			(BITMAP_SET_P(use_spec_F[TID], F_BMAP_SZ, (N))\
				 ? spec_regs_F[TID].d[(N)]			\
				 : regs[TID].regs_F.d[(N)])
#define SET_FPR(TID, N,EXPR)		(spec_mode[TID]				\
				 ? ((spec_regs_F[TID].d[(N)] = (EXPR)),	\
				    BITMAP_SET(use_spec_F[TID],F_BMAP_SZ, (N)),\
				    spec_regs_F[TID].d[(N)])			\
...
/* miscellanous register accessors, NOTE: speculative copy on write storage
   provided for fast recovery during wrong path execute (see tracer_recover()
   for details on this process */
#define FPCR(TID)			(BITMAP_SET_P(use_spec_C[TID], C_BMAP_SZ,/*fpcr*/0)\
				 ? spec_regs_C[TID].fpcr			\
				 : regs[TID].regs_C.fpcr)
#define SET_FPCR(TID, EXPR)		(spec_mode[TID]				\
				 ? ((spec_regs_C[TID].fpcr = (EXPR)),	\
				   BITMAP_SET(use_spec_C[TID],C_BMAP_SZ,/*fpcr*/0),\
				    spec_regs_C[TID].fpcr)			\
				 : (regs[TID].regs_C.fpcr = (EXPR)))
#define UNIQ(TID)			(BITMAP_SET_P(use_spec_C[TID], C_BMAP_SZ,/*uniq*/1)\
				 ? spec_regs_C[TID].uniq			\
				 : regs[TID].regs_C.uniq)
#define SET_UNIQ(TID, EXPR)		(spec_mode[TID]				\
				 ? ((spec_regs_C[TID].uniq = (EXPR)),	\
				   BITMAP_SET(use_spec_C[TID],C_BMAP_SZ,/*uniq*/1),\
				    spec_regs_C[TID].uniq)			\
				 : (regs[TID].regs_C.uniq = (EXPR)))
#define FCC(TID)			(BITMAP_SET_P(use_spec_C[TID], C_BMAP_SZ,/*fcc*/2)\
				 ? spec_regs_C[TID].fcc			\
				 : regs[TID].regs_C.fcc)
#define SET_FCC(EXPR)		(spec_mode[TID]				\
				 ? ((spec_regs_C[TID].fcc = (EXPR)),		\
				    BITMAP_SET(use_spec_C[TID],C_BMAP_SZ,/*fcc*/1),\
				    spec_regs_C[TID].fcc)			\
				 : (regs[TID].regs_C.fcc = (EXPR)))
...
ruu_dispatch(void)
{
  ...
  while (/* instruction decode B/W left? */
	      ...
	      && !OPERANDS_READY(last_op.rs)))
	{
      ...
      valid = fetch_data[fetch_head].valid;
      if (!valid) {
        fetch_head = (fetch_head+1) & (ruu_ifq_size - 1);
        fetch_num--;
        fetch_icount[tid]--;
...
#include "machine.def"
      ...
      if (MD_OP_FLAGS(op) & F_MEM)
	{
      ...
      br_taken = (regs[tid].regs_NPC != (regs[tid].regs_PC + sizeof(md_inst_t))); (void)br_taken;
      br_pred_taken = (pred_PC[tid] != (regs[tid].regs_PC + sizeof(md_inst_t)));

      if ((pred_PC[tid] != regs[tid].regs_NPC && pred_perfect)
	  || ((MD_OP_FLAGS(op) & (F_CTRL|F_DIRJMP)) == (F_CTRL|F_DIRJMP)
	      && target_PC != pred_PC[tid] && br_pred_taken))
	{
    ...
    for (int flush_tid = 0; flush_tid < thread_num; flush_tid++) {
      fetch_pred_PC[flush_tid] = fetch_regs_PC[flush_tid] = regs[flush_tid].regs_NPC;
      fetch_icount[flush_tid] = 0;
      /* was: if (pred_perfect) */
      if (pred_perfect)
        pred_PC[flush_tid] = regs[flush_tid].regs_NPC;
    }
      ...
      if (op != MD_NOP_OP)
	{
	  ...
	  rs->tid = tid;
	  rs->valid = TRUE;
	  rs->PC = regs[tid].regs_PC;
	  rs->next_PC = regs[tid].regs_NPC; rs->pred_PC = pred_PC[tid];
	  rs->in_LSQ = FALSE;
	  rs->ea_comp = FALSE;
	  ...
	  if (MD_OP_FLAGS(op) & F_MEM)
	    {
	      ...
	      lsq->tid = tid;
	      lsq->valid = TRUE;
	      lsq->PC = regs[tid].regs_PC;
	      lsq->next_PC = regs[tid].regs_NPC; lsq->pred_PC = pred_PC[tid];
	      lsq->in_LSQ = TRUE;
	      lsq->ea_comp = FALSE;
	      ...
	      ruu_install_odep(lsq, tid, /* odep_list[] index */1, out2);
	  ...
	  else /* !(MD_OP_FLAGS(op) & F_MEM) */
	    {
	      ...
	      ruu_install_odep(rs, tid, /* odep_list[] index */1, out2);
	      ...
	      if (pred[tid] && bpred_spec_update == spec_ID)
		{
		  bpred_update(pred[tid],
			       ...
			       /* taken? */regs[tid].regs_NPC != (regs[tid].regs_PC +
						       sizeof(md_inst_t)),
			       /* pred taken? */pred_PC[tid] != (regs[tid].regs_PC +
							sizeof(md_inst_t)),
			       /* correct pred? */pred_PC[tid] == regs[tid].regs_NPC,
			       /* opcode */op,
      ...
      for (i=0; i<pcstat_nelt; i++)
	{
	  ...
	  if (delta != 0)
	    {
	      stat_add_samples(pcstat_sdists[i], regs[tid].regs_PC, delta);
...
static int last_inst_missed[MAX_THREAD];
static int last_inst_tmissed[MAX_THREAD];
...
ruu_fetch(void)
{
  ...
  fetch_thread = 0;
  unsigned int min_icount = total_icount[0] = fetch_icount[0] + ruu_icount[0];
  for (int tid = 1; tid < thread_num; tid++) {
    total_icount[tid] = fetch_icount[tid] + ruu_icount[tid];
  ...
  for (i=0, branch_cnt=0;
	  ...
	  && !(fetch_regs_PC[fetch_thread] & (sizeof(md_inst_t)-1)))
	{
	  ...
	  if (cache_il1)
		...
		last_inst_missed[fetch_thread] = TRUE;
	    }
	  ...
	  if (itlb)
		...
		cache_access(itlb, Read, fetch_thread, IACOMPRESS(fetch_regs_PC[fetch_thread]),
			     ...
			     NULL, NULL);
	      if (tlb_lat > 1)
		last_inst_tmissed[fetch_thread] = TRUE;
      ...
      if (pred[fetch_thread])
	{
	  ...
	  if (MD_OP_FLAGS(op) & F_CTRL)
	    fetch_pred_PC[fetch_thread] =
	      bpred_lookup(pred[fetch_thread],
      ...
      else
	{
      ...
      fetch_data[fetch_tail].tid = fetch_thread;
      fetch_data[fetch_tail].valid = TRUE;
      fetch_data[fetch_tail].regs_PC = fetch_regs_PC[fetch_thread];
      fetch_data[fetch_tail].pred_PC = fetch_pred_PC[fetch_thread];
      ...
      ptrace_newstage(fetch_data[fetch_tail].ptrace_seq,
		      PST_IFETCH,
		      ((last_inst_missed[fetch_thread] ? PEV_CACHEMISS : 0)
		       | (last_inst_tmissed[fetch_thread] ? PEV_TLBMISS : 0)));
      last_inst_missed[fetch_thread] = FALSE;
      last_inst_tmissed[fetch_thread] = FALSE;

      /* adjust instruction fetch queue */
      fetch_tail = (fetch_tail + 1) & (ruu_ifq_size - 1);
      fetch_num++;
      fetch_icount[fetch_thread]++;
    }
}
  ...
  if (fastfwd_count > 0)
    {
	  ...
	  /* maintain $r0 semantics */
              regs[tid].regs_R[MD_REG_ZERO] = 0;
#ifdef TARGET_ALPHA
              regs[tid].regs_F.d[MD_REG_ZERO] = 0.0;
#endif /* TARGET_ALPHA */
...
#include "machine.def"
	  ...
	  if (dlite_check_break(regs[fetch_thread].regs_NPC,
	    ...
	    dlite_main(regs[fetch_thread].regs_PC, regs[fetch_thread].regs_NPC, sim_num_insn,
        &regs[fetch_thread], mem[fetch_thread]);
  ...
  for (int tid = 0; tid < thread_num; tid++) {
    fetch_regs_PC[tid] = regs[tid].regs_PC - sizeof(md_inst_t);
    fetch_pred_PC[tid] = regs[tid].regs_PC;
    regs[tid].regs_PC = regs[tid].regs_PC - sizeof(md_inst_t);
  }
  ...
  for (;;)
	...
	panic("LSQ_head/LSQ_tail wedged");
      ...
      ptrace_check_active(regs[fetch_thread].regs_PC, sim_num_insn, sim_cycle);
